...
class Verse:
    ...
    def scoped_title(self, other_verse):
        if self.book_num == other_verse.book_num:
            if self.chapter_num == other_verse.chapter_num:
                return f"{self.verse_num}"
            else:
...
def verse_range_title(bcv1, bcv2):
    v1 = verse_from_bcv(bcv1)
    if bcv1 == bcv2:
        return v1.title
    v2 = verse_from_bcv(bcv2)
    return f"{v1.title}–{v2.scoped_title(v1)}"
